...
const Otp = ({ onClose, onCloseAll, mobileNumber, onComplete }) => {
  ...
  const otpInputRef = useRef(null); // Reference for the OTP input field
  ...
  useEffect(() => {
    // Set focus to the OTP input field when the component mounts
  ...
  useEffect(() => {
    // Automatically submit the form when OTP length is 6
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-10 z-50">
      <div className="bg-white border-[1px] border-gray-300 m-4 p-8 rounded-lg w-[25rem] relative text-center shadow-lg">
        <div className="absolute top-3 left-2 text-right">
          <div
            onClick={onClose}
            className="text-gray-500 w-8 text-[20px] cursor-pointer hover:text-black"
          >
        ...
        >
          <input
            ref={otpInputRef} // Set the ref here for autofocus
            type="text"
            value={otp}
            onChange={(e) => setOtp(e.target.value)}
            className="px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:border-blue-500 bg-transparent text-center"
            maxLength={6}
          />
          <button
            type="submit"
